...
class ResolverStoreCart extends Resolver
{
    ...
    public function get()
    {
        ...
        $cartData = array(
            ...
            'products' => array(),
            'deliveryOption' => $deliveryOption[$this->context->cart->id_address_delivery],
            'vouchers' => $this->formatVouchers($presented_cart['vouchers']),
            'subtotals' => $presented_cart['subtotals'],
            'total' => $presented_cart['totals']['total']
        );
    ...
    public function setDeliveryOption($args)
    {
        $this->context->cart->setDeliveryOption([$this->context->cart->id_address_delivery => $args['option']]);
        $this->context->cart->update();
        return $this->get();
    }

    private function formatVouchers($vouchers)
    {
        $added = [];
        ...
        foreach ($vouchers['added'] as $voucher)
        {
            $added[] = array(
                ...
                'name' => $voucher['name'],
                'code' => $voucher['code'],
                'reduction' => $voucher['reduction_formatted'],
            );
        }

        $data = array(
            'allowed' => $vouchers['allowed'],
            'added' => $added
        );
