...
class ResolverStoreCheckout extends Resolver
{
    ...
    public function createPaymentIntent($args) {
        ...
        $result = $this->model_common_payment->createPaymentIntent(
    ...
    public function confirmOrder($args)
    {
        ...
        $order->id_address_delivery = $args['shippingAddress'];
        $order->id_address_invoice = $args['paymentAddress'];
        ...
        $order->id_carrier =  1;//str_replace(',', '', $delivery_option);
        ...
        // $order->addOrderPayment($order_total, 'Yownes', null, 1);
    ...
    public function orders()
    {
        ...
        $orders = $this->model_common_order->getOrders($this->context->customer->id);
    ...
    public function order($args)
    {
        ...
        $order = $this->model_common_order->getOrder($this->context->customer->id, $args['id']);
        ...
        foreach ($order['products'] as $product) {
            $products[] = array(
                ...
                'product' => $this->load->resolver('store/product/get', array('id' => $product['id_product'])),
                'quantity' => $product['quantity'],
                'total' => Tools::displayPrice($product['total'])
            );
        }

        $order['products'] = $products;

        $order['shippingAddress'] = $this->model_common_address->parseAddress($order['shippingAddress']['id'], $order['shippingAddress']);
        $order['paymentAddress'] = $this->model_common_address->parseAddress($order['paymentAddress']['id'], $order['paymentAddress']);
